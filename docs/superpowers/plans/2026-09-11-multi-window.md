...
**Interfaces:**
- Produces: shared type `PomodoroState { phase: 'focus' | 'break'; running: boolean; remaining: number }`; `getPomodoroState()`, `togglePomodoro()`, `resetPomodoro()`, `startPomodoroTicker(): () => void` (main); `window.oryn.pomodoro.{state,toggle,reset,onTick}` (renderer) — consumed by Task 4 (mini window entry) and Task 5 (`Pomodoro.tsx`).
...
(Just `<Pomodoro />`, no special prop — `openMiniPomodoro()` is idempotent, so the pop-out button Task 5 adds is harmless to show even inside the mini window itself: clicking it there just refocuses the same singleton window.)
...
Same sandboxed-shell limitation. When next running the app: manually invoke `window.oryn.pomodoro.openMini()` from the renderer devtools console (Task 5 wires a real button for this). Confirm a small frameless always-on-top window appears showing the same running clock; start/pause/reset it and confirm the change reaches other windows within half a second once Task 5's `onTick` consumer exists.
...
### Task 5: `Pomodoro.tsx` becomes a thin view

**Files:**
- Modify: `src/renderer/src/components/Pomodoro.tsx`

**Interfaces:**
- Consumes: `window.oryn.pomodoro.{state,toggle,reset,onTick,openMini}` from Tasks 3-4.
- Produces: same public props as before (`{ onLogged?: () => void }` — no new props needed since Task 4 dropped the `showPopOut` idea).

- [ ] **Step 1: Replace the file**

Replace `src/renderer/src/components/Pomodoro.tsx` entirely:
...
```bash
git add src/renderer/src/components/Pomodoro.tsx
git commit -m "Pomodoro.tsx: thin view over the main-process timer, add pop-out button"
```
